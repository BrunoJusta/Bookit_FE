<template>
  <div class="kitDetail">
    <h3 class="display-2" v-bind:style="{display:show3}">{{this.currentWorkshop.name}}</h3>
    ...
    <div class="container">
      <b-card no-body class="overflow-hidden border-0" style="max-width: 1100px;">
        <b-row no-gutters>
          <b-col md="6">
            <b-card-img v-bind:src="this.currentWorkshop.img" class="rounded-0"></b-card-img>
          </b-col>
          <b-col md="6">
            <b-card-body align="left" title="Informação">
              ...
              <div class="row">
                <p v-bind:style="{display:show2}" id="b"> <b>Data:</b> {{this.currentWorkshop.date}} </p>
                <p v-bind:style="{display:show2}" id="b"> <b>Duração:</b> {{this.currentWorkshop.duration}} </p>
              </div>
              <div class="row">
                <p v-bind:style="{display:show2}" id="b"><b>Vagas:</b>
                    {{this.currentWorkshop.vacancies - this.currentWorkshop.filled}} </p>
                <p v-bind:style="{display:show2}" id="b"><b>Locutor:</b> {{this.currentWorkshop.teacher}} </p>
              </div>
              <p v-bind:style="{display:show2}">{{this.currentWorkshop.description}}</p>
              <textarea id="description" rows="4" cols="50" style="resize: none; width: 100%;"
                v-bind:style="{display:show}"></textarea>
            </b-card-body>
          </b-col>
        </b-row>
      </b-card>
      ...
      <b-button v-bind:style="{display:show4}" @click="sendInfo()" class="btn-book border-0" squared>Inscrever
      </b-button>
      <b-button v-bind:style="{display:show2}" v-if="this.userType == 0" @click="activateEdit()"
        class="btn-book border-0" squared>
        Editar
      </b-button>
    </div>
  </div>
