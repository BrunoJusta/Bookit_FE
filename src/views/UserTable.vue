    ...
    <div class="container">
      ...
      >
        <template v-slot:cell(actions)="row">
          <b-button
            size="sm"
            v-if="row.item.type == 'User' && row.item.email != $store.getters.getEmail"
            @click="changeToAdmin(row.item.id)"
            class="mr-1 adminBtn"
          >Tornar administrador</b-button>
          <b-button
            size="sm"
            v-if="row.item.type == 'Admin' && row.item.email != $store.getters.getEmail"
            @click="changeToClient(row.item.id) "
            class="mr-1 userBtn"
          >Tornar cliente</b-button>
          <b-button
            size="sm"
            v-if="row.item.email != $store.getters.getEmail && row.item.type !== 'User'"
            @click="block(row.item.id)"
            class="mr-1 blockBtn"
          >
            Bloquear
          ...
          <b-button
            ...
            v-if="row.item.email != $store.getters.getEmail"
